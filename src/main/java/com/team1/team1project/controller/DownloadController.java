...
public class DownloadController {
    ...
    public void downloadExcel(@PathVariable String type, HttpServletResponse response) throws IOException {
        ...
        fileNames.put("employee", "사원_전체.xlsx");
    ...
    public void downloadCsv(@PathVariable String type, HttpServletResponse response) throws IOException {
        ...
        Map<String, String> fileNames = new HashMap<>();
        fileNames.put("employee", "사원_전체.csv");
    ...
    @GetMapping("/sample/employee")
    public void downloadEmployeeSample(HttpServletResponse response) throws IOException {
        String sample = "employeeName|email|phoneNumber|hireDate|resignationDate|department|position|salary|address\n기존이름|변경이메일|변경연락처|변경입사일|변경퇴사일|변경부서|변경직급|변경급여|변경주소\n";

        String fileName = "사원_업로드_샘플.csv";
