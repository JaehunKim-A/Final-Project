document.addEventListener("DOMContentLoaded", function () {
    ...
    table.addEventListener("click", (e) => {
        ...
        if (deleteBtn) {
            ...
            const name = deleteBtn.dataset.name || "선택된 사원"; // data-name 속성 활용
    ...
    function toggleAccordionRow(row) {
        ...
        accordionFields.forEach(f => {
            console.log(row.querySelector(`.${entity}-${f}`));
    ...
    document.getElementById("downloadVisibleExcel")?.addEventListener("click", () => {
        ...
        const exportConfig = {
            employee: {
                headers: ["코드", "이름", "이메일", "연락처", "입사일", "퇴사일", "부서", "직급", "급여", "주소"],
                classes: ["employee-id", "employee-name", "employee-email", "employee-phoneNumber", "employee-hireDate", "employee-resignationDate", "employee-department", "employee-position", "employee-salary", "employee-address"],
                sheetName: "사원 목록",
                fileName: "사원_현재보기.xlsx"
            }
            // 필요한 다른 entity 추가 가능
        };
