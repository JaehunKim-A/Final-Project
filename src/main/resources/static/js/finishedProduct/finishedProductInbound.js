document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchInboundList(page, keyword = "") {
    ...
    try {
      const response = await fetch("/finished-product/inbound/api/list", {
        method: "POST",
  ...
  window.openEditModal = async (inboundId) => {
    try {
      const response = await fetch(`/finished-product/inbound/api/${inboundId}`);
      const data = await response.json();
      ...
      document.querySelector("#editCompleteTime").value = data.completeTime?.slice(0, 16);
      document.querySelector("#editStatus").value = data.status;
      document.querySelector("#editSupplierId").value = data.supplierId;
  ...
  document.querySelector("#registerForm").addEventListener("submit", async (e) => {
    ...
    try {
      const response = await fetch("/finished-product/inbound/api/register", {
        method: "POST",
        headers: { "Content-Type": "application/json", [header]: token },
  ...
  document.querySelector("#editForm").addEventListener("submit", async (e) => {
    ...
    const payload = {
      ...
      completeTime: form.editCompleteTime.value,
